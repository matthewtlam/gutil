...
absl::Status AddConstraintsPreventingIngressPortBeingInLoopbackMode(
  ...
  const ir::Table *loopback_table =
      gutil::FindOrNull(state.program.tables(), kEgressPortLoopbackTableName);

  // TODO: Return error instead of OkStatus when all config images
  // have this table.
  if (loopback_table == nullptr) {
    LOG(WARNING)
        << "Table '" << kEgressPortLoopbackTableName
        << "' was not found in the SAI-P4 program, skipping adding constraints "
           "preventing ingress port being in loopback mode.";
    return absl::OkStatus();
  }

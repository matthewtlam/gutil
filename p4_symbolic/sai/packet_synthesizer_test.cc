...
// TODO: Remove when constraint preventing packets from ingressing
// on loopback ports in moved to client side rather than being hardcoded.
TEST(PacketSynthesizerTest, CannotSynthesizePacketsForPortsInLoopbackMode) {
  constexpr int kPortCount = 10;
  constexpr int kLoopbackPort = 7;

  PacketSynthesisParams params;
  {
    const sai::Instantiation instantiation = sai::Instantiation::kExperimentalTor;
    const pdpi::IrP4Info ir_p4info = sai::GetIrP4Info(instantiation);
    const p4::v1::ForwardingPipelineConfig pipeline_config =
        sai::GetNonstandardForwardingPipelineConfig(
            instantiation, sai::NonstandardPlatform::kP4Symbolic);
    *params.mutable_pipeline_config() = pipeline_config;
    {
      pdpi::IrEntity ir_entity = gutil::ParseProtoOrDie<pdpi::IrEntity>(R"pb(
        table_entry {
      ...
      ir_entity.mutable_table_entry()
          ->mutable_matches(0)
          ->mutable_exact()
          ->set_str(absl::StrCat(kLoopbackPort));
      const auto pi_entry =
          pdpi::IrEntityToPi(ir_p4info, ir_entity, {.allow_unsupported = true});
      ASSERT_OK(pi_entry);
      *params.add_pi_entities()->mutable_table_entry() =
          pi_entry->table_entry();
    }
  }
  ...
  {
    // Ask to synthesize a packet ingressing on the loopback port.
    ASSERT_OK_AND_ASSIGN(const PacketSynthesisResult result,
                         synthesizer->SynthesizePacket(
                             gutil::ParseProtoOrDie<PacketSynthesisCriteria>(
                                 R"pb(
                                   ingress_port_criteria { v1model_port: 7 }
                                 )pb")));
    // Check that the synthesizer can't synthesize the packet.
    EXPECT_FALSE(result.has_synthesized_packet());
  }
  {
    // Ask to synthesize a packet ingressing on a non-loopback port.
    ASSERT_OK_AND_ASSIGN(const PacketSynthesisResult result,
                         synthesizer->SynthesizePacket(
                             gutil::ParseProtoOrDie<PacketSynthesisCriteria>(
                                 R"pb(
    ...
    // Check that the synthesizer can synthesize the packet.
    EXPECT_TRUE(result.has_synthesized_packet());
  }
}

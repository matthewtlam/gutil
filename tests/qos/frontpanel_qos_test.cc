...
TEST_P(FrontpanelQosTest,
  ...
  if (GetParam().queue_by_dscp.has_value()) {
    // Set test case ID for unicast traffic.
    if (GetParam().nsf_reboot) {
      testbed->Environment().SetTestCaseID(
          "807da3f9-43ce-4f23-b5c6-62a0f06e505c");
    } else {
      testbed->Environment().SetTestCaseID(
          "d748242b-93fe-4cf8-a40e-f5b928909baf");
    }
    ASSERT_OK_AND_ASSIGN(kDscpsByUnicastQueueName,
                         GetQueueToDscpsMapping(*GetParam().queue_by_dscp));
  }
  if (GetParam().multicast_queue_by_dscp.has_value()) {
    // Set test case ID for DSCP classification and scheduler rate limit for
    // multicast traffic.
    testbed->Environment().SetTestCaseIDs(
        {"311d6903-d302-49fd-ba28-362d4aaaf1f9",
         "9f7d5ede-b320-4383-94a0-d585c6990c22"});
    ASSERT_OK_AND_ASSIGN(
...
TEST_P(FrontpanelQosTest, WeightedRoundRobinWeightsAreRespected) {
  ...
  if (GetParam().nsf_reboot) {
    testbed->Environment().SetTestCaseID(
        "d6730b23-fbae-4aa0-babd-b44d4200283b");
  } else {
    testbed->Environment().SetTestCaseID(
        "948298f5-3ebc-4d92-8446-5ee6e8617502");
  }
...
TEST_P(FrontpanelQosTest, StrictQueuesAreStrictlyPrioritized) {
  ...
  if (GetParam().nsf_reboot) {
    testbed->Environment().SetTestCaseID(
        "45e42230-7f8b-41df-a6ec-42bb50633e27");
  } else {
    testbed->Environment().SetTestCaseID(
...
TEST_P(FrontpanelQosTest, TestWredEcnMarking) {
  ...
  if (GetParam().nsf_reboot) {
    testbed->Environment().SetTestCaseID(
        "f5dbe4be-97bb-4a46-a5f2-5d200a7e1a45");
  } else {
    testbed->Environment().SetTestCaseID(
        "1af5a469-6634-467b-a0fd-19eea425a836");
  }
...
TEST_P(FrontpanelBufferTest, BufferCarving) {
  ...
  if (GetParam().default_params.queue_by_dscp.has_value()) {
    // Set test case ID dynamic buffer for unicast traffic.
    if (GetParam().config_to_be_tested == kSharedAlpha) {
      if (GetParam().default_params.nsf_reboot) {
        testbed->Environment().SetTestCaseID(
            "150d2aec-6b30-40b4-9ccd-e65138d62bbc");
      } else {
        testbed->Environment().SetTestCaseID(
  ...
  if (GetParam().default_params.multicast_queue_by_dscp.has_value()) {
    // Set test case ID dynamic buffer for multicast traffic.
    if (GetParam().config_to_be_tested == kSharedAlpha) {
      testbed->Environment().SetTestCaseID(
          "6583098e-c915-45d7-90f1-1882761d2845");
    } else if (GetParam().config_to_be_tested == kDedicatedBuffer) {
      testbed->Environment().SetTestCaseID(
          "b53a6b29-6c13-46ee-b1db-7c1598417f0b");
    }

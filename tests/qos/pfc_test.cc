...
absl::StatusOr<QueueCounters> GetQueueCountersChange(
  ...
  ASSIGN_OR_RETURN(const pins_test::QueueCounters queue_counters_pre,
                   GetGnmiQueueCounters(/*port=*/port,
                                        /*queue=*/queue, gnmi));
  LOG(INFO) << "queue_counters_pre: " << queue_counters_pre;
  absl::SleepFor(absl::Seconds(15));
  // Read counters of the target queue.
  ASSIGN_OR_RETURN(const pins_test::QueueCounters queue_counters_post,
                   GetGnmiQueueCounters(/*port=*/port,
                                        /*queue=*/queue, gnmi));
  LOG(INFO) << "queue_counters_post: " << queue_counters_post;
...
TEST_P(PfcTestWithIxia, PfcRxWithNoPacketDrops) {
  ...
  for (const auto pause_quanta : {0x3ff, 0x1fff, 0xffff}) {
    for (const auto [priority, queue] : *GetParam().queue_by_pfc_priority) {
      ...
      EXPECT_OK(gpins::TryUpToNTimes(5, /*delay=*/absl::Seconds(30), [&] {
        // Read counters of the target queue.
        ASSIGN_OR_RETURN(
      ...
      ASSERT_OK_AND_ASSIGN(
          ...
          GetGnmiPortPfcCounter(
              ...
              absl::StrCat(priority), *gnmi_stub_, PfcCountersType::kPfcRx));

      ASSERT_OK_AND_ASSIGN(const int64_t pfc_on2off_counter_before_traffic,
                           GetGnmiPortPfcCounter(
                               /*port=*/ixia_links_.egress_link.sut_interface,
                               absl::StrCat(priority), *gnmi_stub_,
      ...
      EXPECT_OK(gpins::TryUpToNTimes(5, /*delay=*/absl::Seconds(5), [&] {
        // Read counters of the target queue.
        ASSIGN_OR_RETURN(
...
TEST_P(PfcTestWithIxia, PfcWatchdog) {
  for (auto [prio, queue] : *GetParam().queue_by_pfc_priority) {
    ...
    for (bool create_deadlock : {true, false}) {
      ...
      // Read counters of the target queue.
      pins_test::QueueCounters queue_counters_before_test;
      ASSERT_OK(gpins::TryUpToNTimes(5, /*delay=*/absl::Seconds(30), [&] {
        // Read counters of the target queue.
        ASSIGN_OR_RETURN(
            queue_counters_before_test,
            GetGnmiQueueCounters(/*port=*/ixia_links_.egress_link.sut_interface,
      ...
      ASSERT_OK_AND_ASSIGN(
          ...
          GetGnmiPortPfcCounter(
              /*port=*/ixia_links_.egress_link.sut_interface,
              absl::StrCat(prio), *gnmi_stub_, PfcCountersType::kPfcRx));

      ASSERT_OK_AND_ASSIGN(
          const int64_t pfc_on2off_counter_before_traffic,
          GetGnmiPortPfcCounter(
              /*port=*/ixia_links_.egress_link.sut_interface,
              absl::StrCat(prio), *gnmi_stub_, PfcCountersType::kPfcOn2Off));
      LOG(INFO) << "pfc_rx_counter_before_traffic: "
                << pfc_rx_counter_before_traffic;
      LOG(INFO) << "pfc_on2off_counter_before_traffic: "
                << pfc_on2off_counter_before_traffic;
      ...
      ASSERT_OK_AND_ASSIGN(
          const int64_t pfc_on2off_counter_after_traffic,
          GetGnmiPortPfcCounter(
              /*port=*/ixia_links_.egress_link.sut_interface,
              absl::StrCat(prio), *gnmi_stub_, PfcCountersType::kPfcOn2Off));
      LOG(INFO) << "pfc_rx_counter_after_traffic: "
                << pfc_rx_counter_after_traffic;
      LOG(INFO) << "pfc_on2off_counter_after_traffic: "
                << pfc_on2off_counter_after_traffic;
      ASSERT_OK_AND_ASSIGN(
      ...
      if (create_deadlock) {
        ...
        ASSERT_OK_AND_ASSIGN(QueueCounters queue_counters_delta,
                             GetQueueCountersChange(
                                 /*port=*/ixia_links_.egress_link.sut_interface,
                                 /*queue=*/queue, *gnmi_stub_));
        ASSERT_OK(gpins::TryUpToNTimes(3, /*delay=*/absl::Seconds(5), [&] {

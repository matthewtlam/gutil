...
TEST_P(CpuQosTestWithoutIxia, PerEntryAclCounterIncrementsWhenEntryIsHit) {
  ...
  if (GetParam().nsf_reboot) {
    Testbed().Environment().SetTestCaseID(
        "a5126124-f99d-4464-822e-1496298c8d81");
  } else {
    Testbed().Environment().SetTestCaseID(
        "cfd7e8aa-6521-4683-9c07-038ea146934d");
  }
...
TEST_P(CpuQosTestWithoutIxia, PuntToCpuWithVlanTag) {
  ...
  if (GetParam().nsf_reboot) {
    Testbed().Environment().SetTestCaseID(
        "decc78e3-09e0-4f7c-b23b-565f8c5993ab");
  } else {
    Testbed().Environment().SetTestCaseID(
        "b7b742a8-8418-405c-951f-df4e88064749");
  }
...
TEST_P(CpuQosTestWithoutIxia, TrafficToSwitchInbandGetsMappedToCorrectQueues) {
  ...
  if (GetParam().nsf_reboot) {
    Testbed().Environment().SetTestCaseID(
        "57871736-c321-49d0-aee3-2e466590163f");
  } else {
    Testbed().Environment().SetTestCaseID(
        "61bb0173-0c49-4067-b15a-5c3dd7823126");
  }
...
TEST_P(CpuQosTestWithoutIxia, P4CpuQueueMappingByNameIsCorrect) {
  ...
  if (GetParam().nsf_reboot) {
    Testbed().Environment().SetTestCaseID(
        "18cfd8b2-154b-4cae-b51f-8bdc737348a9");
  } else {
    Testbed().Environment().SetTestCaseID(
        "c2c65288-aad9-4f48-b276-37651c25a684");
  }
  ...
  for (const auto &[queue, setup] : test_cases) {
    ...
    for (int i = 0; i < setup.packets; ++i) {
      EXPECT_OK(gpins::InjectEgressPacket(
...
TEST_P(CpuQosTestWithIxia, TestCPUQueueAssignmentAndQueueRateLimit) {
  ...
  if (GetParam().nsf_reboot) {
    generic_testbed->Environment().SetTestCaseID(
        "e6ad3927-a59a-4dd1-8ba6-f1f71b400026");
  } else {
    generic_testbed->Environment().SetTestCaseID(
...
TEST_P(CpuQosTestWithIxia, TestPuntFlowRateLimitAndCounters) {
  ...
  ASSERT_OK(gpins::SetSflowConfigEnabled(gnmi_stub.get(), /*enabled=*/false));

  absl::Cleanup cleanup([&] {
    // Restore sflow enable config.
    ASSERT_OK_AND_ASSIGN(bool sflow_enabled,
                         gpins::IsSflowConfigEnabled(sut_gnmi_config));
    EXPECT_OK(gpins::SetSflowConfigEnabled(gnmi_stub.get(), sflow_enabled))
  ...
  for (auto &[queue_name, queue_info] : queues) {
    ...
    for (auto acl_table_punt_action :
      ...
      if (acl_table_punt_action.rate_limit_action == kAclTrap ||
        ...
        // Set test case ID for non "deny" actions.
        if (GetParam().nsf_reboot) {
          generic_testbed->Environment().SetTestCaseID(
              "c01a8a73-9137-42fc-8f90-384603ad0a66");
        } else {
          generic_testbed->Environment().SetTestCaseID(
        ...
        // Set test case ID for deny actions.
        if (GetParam().nsf_reboot) {
          generic_testbed->Environment().SetTestCaseID(
              "fb1119c9-f652-4a16-bfc7-e362d3427f2f");
        } else {
          generic_testbed->Environment().SetTestCaseID(
              "29f50292-1a4f-42c7-8f0b-a0be94b19417");
        }
      }
...
TEST_P(CpuQosTestWithIxia, CpuQosBurstyTraffic) {
  ...
  if (GetParam().nsf_reboot) {
    generic_testbed->Environment().SetTestCaseID(
        "d38d0e3e-adee-4b5c-bc7c-6546a0d64d8d");
  } else {
    generic_testbed->Environment().SetTestCaseID(
        "b4d33b7a-39ca-4b2c-93c1-312730fb4844");
  }

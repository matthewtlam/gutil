...
std::string DescribeResults(const std::vector<gpins::GroupMember>& members,
...
int PercentErrorTestPacketCount(int total_weight) {
  ...
  if (packet_count < 1000) return 1000 - 1000 % total_weight + total_weight;
  // Choose the last multiple of packet_count < 10,000
  if (packet_count > 10000) return 10000 - 10000 % total_weight;
  return packet_count;
}

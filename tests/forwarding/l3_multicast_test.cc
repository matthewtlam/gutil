...
// TODO: Increase back to 505 multicast groups.
constexpr int kMaxMulticastGroups = 215;
...
absl::StatusOr<std::vector<dvaas::PacketTestVector>> BuildInputOutputVectors(
  ...
  int unique_payload_ids = 2 * input_group_index + 1;
  ...
  ASSIGN_OR_RETURN(const auto ipv6_address,
                   GetIpv6AddressForReplica(input_group_index));
  ...
  for (int m = 0; m < number_multicast_groups; ++m) {
    ASSIGN_OR_RETURN(
        ...
        BuildInputOutputVectors(port_ids,
                                /*input_group_index=*/m,
                                /*output_group_index=*/m, replicas_per_group));
...
TEST_P(L3MulticastTestFixture, UnregisteredParticipantProgramming) {
  ...
  ASSERT_OK_AND_ASSIGN(
      auto rif_entities,
      CreateRifTableEntities(ir_p4info_, sut_ports_ids[1], kDropInstance,
                             kDropSrcMacAddress));
...
TEST_P(L3MulticastTestFixture, ReplicatingNTimesToSamePortProducesNCopies) {
  GetParam().mirror_testbed->GetMirrorTestbed().Environment().SetTestCaseID(
      "a8a99a6c-0dfc-48bc-a7a0-75cf07925605");
...
TEST_P(L3MulticastTestFixture, AddMulticastReplicaForUnknownPortInstanceFails) {
  ...
  ASSERT_OK_AND_ASSIGN(
      ...
      CreateRifTableEntities(ir_p4info_, sut_ports_ids[0], kDefaultInstance,
                             kOriginalSrcMacAddress));

...
Distribution ExpectedDistribution(const std::vector<gpins::GroupMember> members,
                                  int packet_count) {
...
Distribution DistributionWithError(
    const std::vector<gpins::GroupMember> members, int target_packet_count,
    double error) {
  Distribution distribution =
...
class DistributionTest
    : public testing::TestWithParam<std::vector<gpins::GroupMember>> {};

TEST_P(DistributionTest, ChiSquaredTestPacketCountUsesAverageErrorAsThreshold) {
  constexpr double kConfidence = 0.9;
  constexpr double kAverageError = 0.1;

  const std::vector<gpins::GroupMember>& members = GetParam();
...
std::vector<std::vector<gpins::GroupMember>> TestDistributions() {
  return {
      {
          {.weight = 1, .port = 1},
          {.weight = 2, .port = 2},

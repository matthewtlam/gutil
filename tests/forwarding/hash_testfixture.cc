...
absl::Status SendPacket(const pdpi::IrP4Info& ir_p4info,
  ...
  RETURN_IF_ERROR(gpins::InjectEgressPacket(
      ingress_port_id.GetP4rtEncoding(), raw_packet, ir_p4info,
      &control_p4_session, kPacketInterval))
...
const TestConfigurationMap& NonHashingTestConfigs() {
  static const auto add_if_valid = [](TestConfiguration config,
    ...
    if (gpins::IsValidTestConfiguration(config)) {
...
void HashTest::InitializeTestbed() {
  ...
  ASSERT_OK(pdpi::InstallIrTableEntry(
      ...
      gpins::PuntAllPacketsToControllerIrTableEntry("0x7")));
}
...
void HashTest::TearDown() {
  ...
  switch (mirror_testbed_teardown_condition_) {
    ...
    case TearDownCondition::kOnFailure:
      if (HasFailure()) mirror_testbed_->TearDown();
      break;
  }
}
...
absl::StatusOr<std::vector<packetlib::Packet>> HashTest::GeneratePackets(
    const gpins::TestConfiguration& test_config, int num_packets,
  ...
  // values.
  int range = gpins::Range(test_config);
  ...
  LOG(INFO) << "Generating " << num_packets << " "
            ...
            << " packets for config: " << gpins::DescribeTestConfig(test_config)
  ...
  for (int packet_index = 0; packet_index < num_packets; ++packet_index) {
    int generator_index = use_random_index
                              ...
                              : packet_index;
    ASSIGN_OR_RETURN(packetlib::Packet packet,
                     gpins::GenerateIthPacket(test_config, generator_index));
    SetPayload(
        packet, packet_index,
...
void HashTest::ForwardAllPacketsToMembers(
  ...
  ASSERT_OK(gpins::ProgramNextHops(testbed.Environment(), sut_p4_session(),
                                   ir_p4info, members));

  ASSERT_OK(gpins::ProgramGroupWithMembers(
...
absl::StatusOr<std::string> HashTest::GnmiInterfaceName(
    const P4rtPortId& port_id) const {

...
absl::Status MatchFieldReferenceOverride(absl::BitGen* gen,
  ...
  for (auto& match : *entry.mutable_match()) {
    ...
    if (auto it = reference_map.find(match_field_name);
      ...
      // TODO: b/324943837 - Add check when creating a FuzzerConfig
      switch (match_field_info.match_field().match_type()) {
...
Update::Type FuzzUpdateType(absl::BitGen* gen, const FuzzerConfig& config,
  ...
  if (!is_multicast) {
    // The probability of inserting a entry is larger that the probability of
    ...
    // TODO: b/338472422 - Remove hard cap once defragmentation errors are
    // resolved.
    if ((absl::Bernoulli(*gen, 0.7) ||
         state.GetMulticastGroupEntries().empty()) &&
        state.GetMulticastGroupEntries().size() < 64) {
      return Update::INSERT;
    }
...
AnnotatedUpdate FuzzUpdate(absl::BitGen* gen, const FuzzerConfig& config,
  ...
  bool fuzz_multicast_update =
      !do_mutate &&
      !IsDisabledForFuzzing(config, pdpi::GetMulticastGroupTableName()) &&
...
absl::StatusOr<std::string> FuzzValue(
  ...
  // TODO: b/346813704 - Add proper support for Unicast and Multicast Qos
  // Queues.

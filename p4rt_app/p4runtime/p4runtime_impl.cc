...
std::string GetKeyErrorMessage(const pdpi::IrEntity& entity,
...
absl::StatusOr<EntityUpdate> PiUpdateToEntityUpdate(
  ...
  RETURN_IF_ERROR(UpdateIrEntityForOrchAgent(
      *ir_entity, p4_info, translate_port_ids, port_translation_map,
  ...
  return EntityUpdate{
      .entry = std::move(*ir_entity),
      .update_type = pi_update.type(),
      .pi_entity = std::move(*normalized_pi_entry),
      .entity_key = std::move(entity_key),
      .app_db_update = std::move(app_db_update),
  };
}
...
std::vector<EntityUpdate> PiEntityUpdatesToIr(
  ...
  for (const p4::v1::Update& pi_update : request.updates()) {
    ...
    update->resource_utilization_change = std::move(*resource_change);
...
absl::StatusOr<ConfigInfo> PreprocessConfig(
...
P4RuntimeImpl::P4RuntimeImpl(
  ...
  // If we have an initialization issue then immediately go critical, otherwise
...
grpc::Status P4RuntimeImpl::EnterCriticalState(const std::string& message) {
  ...
  component_state_.ReportComponentState(swss::ComponentState::kError, message);
...
absl::Status P4RuntimeImpl::VerifyState(bool update_component_state) {
  ...
  if (failures.size() > 1) {
    ...
    for (const auto& message : failures) {
      LOG(ERROR) << "State verification failure: " << message;
    }
...
grpc::Status P4RuntimeImpl::ReconcileAndCommitPipelineConfig(
  ...
  // Check the new p4info for compatibility with existing entities.
  for (const auto& [key, entity] : entity_cache_) {
    auto equivalent = IrP4InfoGeneratesEquivalentAppDbRepresentation(
        *ir_p4info_, config_info->ir_p4info, entity, config_info->constraints,
        translate_port_ids_, port_translation_map_, *cpu_queue_translator_,
        *front_panel_queue_translator_);
    if (!equivalent.ok()) {
      LOG(WARNING) << "ForwardingPipelineConfig cannot be reconciled: "
                   << equivalent;
      return gutil::AbslStatusToGrpcStatus(equivalent);
    }
  }
...
absl::Status P4RuntimeImpl::HandleWarmBootNotification(
  ...
  if (notification == swss::WarmStart::WarmBootNotification::kFreeze) {
    ...
    if (warm_boot_state != swss::WarmStart::WarmStartState::COMPLETED &&
        warm_boot_state != swss::WarmStart::WarmStartState::RECONCILED) {
    ...
    LOG(INFO) << "Unfreezing P4RT App, updating warm-boot state from "
              << swss::WarmStart::warmStartStateNameMap()->at(warm_boot_state)
              << " to COMPLETED";
    ...
    UpdateWarmBootState(swss::WarmStart::WarmStartState::COMPLETED);
  }

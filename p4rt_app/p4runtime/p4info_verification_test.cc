...
// Hash algorithm has been removed from P4Info. Keep this test around to
// document expected stack behavior when hash algorithm is provided.
TEST(P4InfoVerificationTest,
     DISABLED_ReturnsErrorWhenHashValueVerificationFails) {
  p4::config::v1::P4Info p4info =
      sai::GetP4Info(sai::Instantiation::kMiddleblock);
...
TEST(P4InfoVerificationTest, ToleratesUnsupportedAclAction) {
  ...
  SCOPED_TRACE(absl::StrCat("action: ", action->preamble()));

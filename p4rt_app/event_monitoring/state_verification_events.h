...
class StateVerificationEvents {
 public:
  ...
  // Waits on a table update from STATE_DB on the state verification events.
  // P4RT waits for an explicit ready/pass/fail event from Orchagent to start
  // its state verification, times out if no updates in 3 seconds.
  absl::Status WaitForReadyEvent(absl::string_view verify_req_timestamp);
 ...
 private:
  ...
  sonic::SubscriberStateTableAdapter& subscriber_table_
      ABSL_GUARDED_BY(event_lock_);

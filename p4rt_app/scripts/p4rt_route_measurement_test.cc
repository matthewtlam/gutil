...
struct MulticastEntryInfo {
  ...
  std::vector<gpins::MulticastReplica> replicas;
};
...
absl::Status GenerateRandomMulticastRIFs(absl::BitGen& bitgen,
  ...
  for (const auto& address : addresses) {
    ...
    gpins::MulticastReplica replica =
        gpins::MulticastReplica(port_name, instance, mac.ToString());
    ASSIGN_OR_RETURN(entries.rifs_by_name[replica.key],
                     gpins::MulticastRouterInterfaceTableUpdate(
...
absl::StatusOr<P4WriteRequests> ComputeIpMulticastWriteRequests(
  ...
  for (const uint16_t group_id : multicast_group_ids) {
    ...
    std::vector<gpins::MulticastReplica> replicas_to_use;
    ASSIGN_OR_RETURN(
    ...
    // The initial INSERT request.
    absl::Span<gpins::MulticastReplica> replicas_span{replicas_to_use};
    ASSIGN_OR_RETURN(
        *requests.inserts.back().add_updates(),
        gpins::MulticastGroupUpdate(ir_p4info, p4::v1::Update::INSERT, group_id,
                                    replicas_span));

    // MODIFY replicas to use.
    std::vector<gpins::MulticastReplica> replicas_to_use_modify;
    ASSIGN_OR_RETURN(
    ...
    absl::Span<gpins::MulticastReplica> replicas_modify_span{
        replicas_to_use_modify};
    ASSIGN_OR_RETURN(
        *requests.modifies.back().add_updates(),
        gpins::MulticastGroupUpdate(ir_p4info, p4::v1::Update::MODIFY, group_id,
                                    replicas_modify_span));

    // DELETE the entry.
    ASSIGN_OR_RETURN(
        *requests.deletes.back().add_updates(),
        gpins::MulticastGroupUpdate(ir_p4info, p4::v1::Update::DELETE, group_id,
                                    replicas_modify_span));
  }

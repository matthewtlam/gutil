...
sonic::VrfTable CreateVrfTable(swss::DBConnector* app_db,
  ...
  return sonic::VrfTable{
      ...
      .notification_consumer =
          absl::make_unique<sonic::ConsumerNotifierAdapter>(kVrfResponseChannel,
                                                            app_db),
      .app_db = absl::make_unique<p4rt_app::sonic::TableAdapter>(
          app_db, APP_VRF_TABLE_NAME),
...
int main(int argc, char** argv) {
  ...
  p4rt_app::P4RuntimeImpl p4runtime_server(
      ...
      std::make_unique<p4rt_app::sonic::WarmBootStateAdapter>(),
      // LINT.IfChange
      std::move(packetio_impl), component_state_singleton,
      system_state_singleton, netdev_translator,
      // LINT.ThenChange(//depot/google3/third_party/pins_infra/p4rt_app/p4runtime/p4runtime_impl.cc:reconciliation_precondition)
      p4rt_options);

  p4rt_app::sonic::StateEventMonitor oa_reconcile_monitor(&state_db);
  bool monitor_oa_reconcile_events = true;
  bool oa_reconciled = false;
  std::thread oa_reconcile_listener_thread;
  ...
  if (is_warm_start) {
    ...
    if (reconciliation_status.ok()) {
      ...
      if (!warm_restart_util.ShouldWaitForGlobalUnfreeze()) {
        // Start a thread listening to OA warm boot state, unfreeze P4RT once OA
        // is reconciled.
        oa_reconcile_listener_thread = std::thread(
            absl::bind_front(&p4rt_app::WaitForOrchAgentReconcileToUnfreezeP4rt,
                             &oa_reconcile_monitor, &p4runtime_server,
  ...
  // LINT.ThenChange(//depot/google3/third_party/pins_infra/p4rt_app/tests/warm_restart_bootup_test.cc:bootup)
  ...
  p4rt_app::sonic::SubscriberStateTableAdapter subscriber_state_table(
      &state_verification_db, "VERIFY_STATE_RESP_TABLE");
  p4rt_app::StateVerificationEvents state_verification_event_monitor(
      p4runtime_server, state_verification_notifier, subscriber_state_table,

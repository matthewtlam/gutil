...
// unexpected key value then this function will return a failure. The bool
// return value indicates if every key had a OK response (successfully
// programmed) or not.
//
// NOTE: On failue the key_to_status_map should not be trusted.
absl::StatusOr<bool> GetAndProcessResponseNotification(
...
// OrchAgent response it will not try to revert any state. The bool return
// value indicates if every key had a OK response.
absl::StatusOr<bool> GetAndProcessResponseNotificationWithoutRevertingState(

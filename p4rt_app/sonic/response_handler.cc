...
absl::StatusOr<bool> UpdateResponsesAndRestoreState(
    absl::btree_map<std::string, pdpi::IrUpdateStatus*>& key_to_status_map,
  ...
  while (expected_iter != key_to_status_map.end() &&
    ...
    if (expected_key < response_key) {
      ...
      // handle it correctly then we may need to cleanup state in the AppDb.
...
absl::StatusOr<pdpi::IrUpdateStatus> GetAndProcessResponseNotification(
  ...
  RETURN_IF_ERROR(
      GetAndProcessResponseNotification(notification_interface, app_db_table,
                                        state_db_table, key_to_status_map)
          .status());

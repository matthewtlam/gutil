...
@p4runtime_role(P4RUNTIME_ROLE_ROUTING)
  ...
  @entry_restriction("
    ...
    ipv4_dst::value >= ipv4('224.0.0.0');
    ipv4_dst::value <= ipv4('239.255.255.255');
  ")
  ...
  @entry_restriction("
    ...
    ipv6_dst::value >= ipv6('ff00::');
    ipv6_dst::value <= ipv6('ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff');
  ")
  ...
  apply {
    ...
    bool decapped_packet = local_metadata.admit_to_l3 &&
         local_metadata.tunnel_termination_table_hit;
    if (headers.ipv4.isValid()) {
      if (IS_MULTICAST_IPV4(headers.ipv4.dst_addr)) {
        // The dst MAC must either be an MC address or the packet was decapped
        // and the dst MAC will be updated to an MC address later.

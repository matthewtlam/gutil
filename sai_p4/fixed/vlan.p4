...
control ingress_vlan_checks(inout headers_t headers,
                            inout local_metadata_t local_metadata,
                            inout standard_metadata_t standard_metadata) {
  port_id_t port = (port_id_t)standard_metadata.ingress_port;
  ...
  @id(V1MODEL_AUXILIARY_VLAN_MAKE_TAGGED_MEMBER_ACTION_ID)
  action v1model_auxiliary_make_tagged_member() {
    local_metadata.ingress_port_is_member_of_vlan = true;
  }

  @id(V1MODEL_AUXILIARY_VLAN_MAKE_UNTAGGED_MEMBER_ACTION_ID)
  action v1model_auxiliary_make_untagged_member() {
    local_metadata.ingress_port_is_member_of_vlan = true;
  }

  // In SAI, VLAN port membership affects both ingress and egress ports. However
  // in v1model targets (e.g. BMv2) we cannot apply the same table
  // (`vlan_membership_table` in this case) more than once to the same packet.
  // To mimic the symmetrical effect of VLAN membership for these targets, we
  // use two tables: `vlan_membership_table` and
  // `v1model_auxiliary_vlan_membership_table`. From the perspective of the SDN
  // controller and SAI, `vlan_membership_table` is the only table that impacts
  // vlan membership for both the ingress and egress. For v1model targets, for
  // each entry in `vlan_membership_table`, a corresponding auxiliary entry must
  // be added to `v1model_auxiliary_vlan_membership_table`.
  @p4runtime_role(P4RUNTIME_ROLE_V1MODEL_AUXILIARY_CONTROLLER)
  @id(V1MODEL_AUXILIARY_VLAN_MEMBERSHIP_TABLE_ID)
  @unsupported
  table v1model_auxiliary_vlan_membership_table {
    key = {
    ...
    actions = {
      @proto_id(1) v1model_auxiliary_make_tagged_member();
      @proto_id(2) v1model_auxiliary_make_untagged_member();
  ...
  apply {
    ...
    v1model_auxiliary_vlan_membership_table.apply();
  }

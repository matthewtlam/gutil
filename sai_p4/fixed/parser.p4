...
parser packet_parser(packet_in packet, out headers_t headers,
 ...
 state parse_8021q_vlan {
  ...
  state parse_psp {
    packet.extract(headers.psp);

    transition select(headers.psp.crypt_offset, headers.psp.next_header) {
      (0x02, IP_PROTOCOL_UDP): parse_udp_in_psp;
      _: accept;
    }
  }

  state parse_udp_in_psp {
    packet.extract(headers.udp_in_psp);
    transition accept;
  }
...
// across platforms.
    ...
    packet.emit(headers.psp);
    packet.emit(headers.udp_in_psp);
  }

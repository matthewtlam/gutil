...
// Note that PSP has an optional "virtualization cookie" that we do not model
// here. It's only present when PSP is using tunnel mode (i.e. V=1).
header psp_t {
  bit <8> next_header;
  bit <8> hdr_ext_len;
  bit <2> reserved_0;
  bit <6> crypt_offset;
  // We need our ACL rules to match on on the 'vert cookie' field, but because
  // PSP is not standard on most hardware we will be using a UDF match. Since
  // UDF sizes byte offset we combine the following fields into psp_info:
  //   Sample, Drop, Version, VertCookie
  bit <8> psp_info;
  bit <32> security_parameters_index;
  bit <64> initialization_vector;
  // varbit<64> virtualization_cookie;
}

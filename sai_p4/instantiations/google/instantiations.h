...
enum class Instantiation {
  ...
  kMiddleblock,
  kTor,
  // TODO: Due to the uncertainty of the final state of Experimental, we
  // introduced a new instantiation to quickly make progress if it needs to
  // diverge from the standard ToR instantiation. Ideally, these should not
  // diverge and/or we should be able to combine them again in a year when
  // Experimental is no longer in flux.
  kExperimentalTor,
...
inline std::vector<Instantiation> AllSaiInstantiations() {
  return {
      ...
      Instantiation::kTor,
      Instantiation::kExperimentalTor,
  };
}
...
inline std::string InstantiationToString(Instantiation role) {
  switch (role) {
    ...
    case Instantiation::kTor:
      return "tor";
    case Instantiation::kExperimentalTor:
      return "experimental_tor";

...
enum VlanTaggingMode { kTagged, kUntagged };
...
pdpi::IrEntities GetVlanMembershipEntities(
    std::vector<VlanMember> vlan_members) {
  pdpi::IrEntities ir_entities;

  for (const auto& vlan_member : vlan_members) {
    pdpi::IrEntity& aux_ir_entity = *ir_entities.add_entities();
    aux_ir_entity = gutil::ParseProtoOrDie<pdpi::IrEntity>(
        R"pb(
          table_entry {
            table_name: "v1model_auxiliary_vlan_membership_table"
            matches {
              name: "vlan_id"
              exact { hex_str: "TBD" }  # Set below.
            }
            matches {
              name: "port"
              exact { str: "TBD" }  # Set below.
            }
            action { name: "TBD" }  # Set below.
          }
        )pb");
    aux_ir_entity.mutable_table_entry()
        ->mutable_matches()
        ->at(0)
        .mutable_exact()
        ->set_hex_str(vlan_member.vlan_id);
    aux_ir_entity.mutable_table_entry()
        ->mutable_matches()
        ->at(1)
        .mutable_exact()
        ->set_str(vlan_member.port);
    if (vlan_member.tagging_mode == kTagged) {
      *aux_ir_entity.mutable_table_entry()->mutable_action()->mutable_name() =
    ...
    ir_entity = gutil::ParseProtoOrDie<pdpi::IrEntity>(
        R"pb(
          table_entry {
...
TEST(ExperimentalTorVlanTest, VrfTableMatchOnVlanIdMatchesVlanPacketWithThatId) {
  ...
  ASSERT_OK(InstallEntriesOnlyForwardingPacketsMatchingVlanIdInAclPreIngress(
      bmv2, kIrP4Info,
...
TEST(ExperimentalTorVlanTest,
     VlanIdSetInPreIngressAclGoesToEgressIfNexthopVlanRewriteIsDisabled) {
  const sai::Instantiation kInstantiation = sai::Instantiation::kExperimentalTor;
  const pdpi::IrP4Info kIrP4Info = sai::GetIrP4Info(kInstantiation);
  ASSERT_OK_AND_ASSIGN(Bmv2 bmv2, sai::SetUpBmv2ForSaiP4(kInstantiation));

  constexpr absl::string_view kIngressVlan = "0x00c";
  ...
  ASSERT_OK(InstallEntries(
      bmv2, kIrP4Info,
      sai::EntryBuilder().AddEntrySettingVlanIdInPreIngress(kIngressVlan)));

  {
    // Inject packet without a VLAN tag.
    ASSERT_OK_AND_ASSIGN(PacketsByPort output_by_port,
                         bmv2.SendPacket(kIngressPort, GetIpv4PacketOrDie()));
    // The packet must be forwarded with no VLAN tag (ACL_PRE_INGRESS's
    // SET_OUTER_VLAN_ID only affects input packets with a VLAN tag).
    ASSERT_THAT(output_by_port, ElementsAre(Key(kEgressPort)));
    ASSERT_THAT(output_by_port.at(kEgressPort).packets().at(0).headers(),
                ElementsAre(HasHeaderCase(packetlib::Header::kEthernetHeader),
                            HasHeaderCase(packetlib::Header::kIpv4Header)));
  }
  {
    ...
    ASSERT_OK_AND_ASSIGN(
        ...
        bmv2.SendPacket(kIngressPort, GetVlanIpv4PacketOrDie(
                                          /*vid_hexstr=*/"0x00b")));
    // The packet must be forwarded with VLAN 0x00c.
    ASSERT_THAT(output_by_port, ElementsAre(Key(kEgressPort)));
    ASSERT_THAT(output_by_port.at(kEgressPort)
                    .packets()
                    .at(0)
...
TEST(ExperimentalTorVlanTest, NexthopOverridesVlanIdSetInPreIngressAclByDefault) {
  const sai::Instantiation kInstantiation = sai::Instantiation::kExperimentalTor;
  const pdpi::IrP4Info kIrP4Info = sai::GetIrP4Info(kInstantiation);
  ASSERT_OK_AND_ASSIGN(Bmv2 bmv2, sai::SetUpBmv2ForSaiP4(kInstantiation));

  constexpr absl::string_view kIngressVlan = "0x00c";
  // Disable VLAN check, enable VLAN rewrite to the default, and set VLAN id in
  // pre-ingress.
  ASSERT_OK(InstallEntriesForwardingAndRewritingVlanInRifTable(
      bmv2, kIrP4Info,
      /*egress_rif_vlan=*/std::nullopt, kEgressPortProto,
      /*disable_vlan_checks=*/true, /*disable_vlan_rewrite=*/false));
  ASSERT_OK(InstallEntries(
      bmv2, kIrP4Info,
      sai::EntryBuilder().AddEntrySettingVlanIdInPreIngress(kIngressVlan)));
...
TEST(ExperimentalTorVlanTest, MatchOnVlanIdInPreIngressAclWorksAsExpected) {
  const sai::Instantiation kInstantiation = sai::Instantiation::kExperimentalTor;
  const pdpi::IrP4Info kIrP4Info = sai::GetIrP4Info(kInstantiation);
  ASSERT_OK_AND_ASSIGN(Bmv2 bmv2, sai::SetUpBmv2ForSaiP4(kInstantiation));

  constexpr absl::string_view kPreIngressMatchVlan = "0x00a";
...
TEST(ExperimentalTorVlanTest, PacketWithoutVlanTagMatchesPreIngressMatchOnVlan4095) {
  const sai::Instantiation kInstantiation = sai::Instantiation::kExperimentalTor;
  const pdpi::IrP4Info kIrP4Info = sai::GetIrP4Info(kInstantiation);
  ASSERT_OK_AND_ASSIGN(Bmv2 bmv2, sai::SetUpBmv2ForSaiP4(kInstantiation));
...
TEST(ExperimentalTorVlanTest, VlanPreservedForPuntedPackets) {
  const sai::Instantiation kInstantiation = sai::Instantiation::kExperimentalTor;
  const pdpi::IrP4Info kIrP4Info = sai::GetIrP4Info(kInstantiation);
  ASSERT_OK_AND_ASSIGN(Bmv2 bmv2, sai::SetUpBmv2ForSaiP4(kInstantiation));
...
// Only test ExperimentalToR instantiation since only it supports
...
TEST(VlanMembershipTest, VlanIngressFilteringWithVlanChecksEnabled) {
  const sai::Instantiation kInstantiation = sai::Instantiation::kExperimentalTor;
  const pdpi::IrP4Info kIrP4Info = sai::GetIrP4Info(kInstantiation);
  ASSERT_OK_AND_ASSIGN(Bmv2 bmv2, sai::SetUpBmv2ForSaiP4(kInstantiation));

  absl::string_view kMemberVlanId = "0x00a";     // VID 10.
  absl::string_view kNonMemberVlanId = "0x014";  // VID 20.
  ...
  ASSERT_OK_AND_ASSIGN(
      ...
      pdpi::IrEntitiesToPi(
          kIrP4Info,
          GetVlanMembershipEntities({{
              .vlan_id = kMemberVlanId,
              .port = kIngressPortProto,
              .tagging_mode = VlanTaggingMode::kTagged,
          }}),
          pdpi::TranslationOptions{.allow_unsupported = true}));
  ASSERT_OK(pdpi::InstallPiEntities(bmv2.P4RuntimeSession(), pi_entities));

  {
    // Input packet does not have a VLAN tag, so the packet should be forwarded.
    ASSERT_OK_AND_ASSIGN(auto outputs,
                         bmv2.SendPacket(kIngressPort, GetIpv4PacketOrDie()));
    ASSERT_THAT(outputs, ElementsAre(Key(kEgressPort)));
    ASSERT_THAT(outputs[kEgressPort].packets(), SizeIs(1));
  ...
  {
    // Ingress port is a member of ingress VLAN, so the packet should be
    // forwarded.
    ASSERT_OK_AND_ASSIGN(
        auto outputs,
        bmv2.SendPacket(kIngressPort,
                        GetVlanIpv4PacketOrDie(/*vid_hexstr=*/kMemberVlanId)));
    ASSERT_THAT(outputs, ElementsAre(Key(kEgressPort)));
    ASSERT_THAT(outputs[kEgressPort].packets(), SizeIs(1));
  }

  {
    // Ingress port is NOT a member of ingress VLAN, so the packet should be
    // dropped.
    ASSERT_OK_AND_ASSIGN(
        auto outputs,
        bmv2.SendPacket(kIngressPort, GetVlanIpv4PacketOrDie(
                                          /*vid_hexstr=*/kNonMemberVlanId)));
    ASSERT_THAT(outputs, IsEmpty());
  }
}

TEST(VlanMembershipTest,
     EgressPacketIsTaggedIfEgressPortIsTaggedMemberOfVlanInEgress) {
  const sai::Instantiation kInstantiation = sai::Instantiation::kExperimentalTor;
  const pdpi::IrP4Info kIrP4Info = sai::GetIrP4Info(kInstantiation);
  ASSERT_OK_AND_ASSIGN(Bmv2 bmv2, sai::SetUpBmv2ForSaiP4(kInstantiation));

  constexpr absl::string_view kIngressVlanId = "0x014";  // VID 20.

  // Carry over VLAN in ingress to egress.
  ASSERT_OK(InstallEntriesForwardingAndRewritingVlanInRifTable(
      bmv2, kIrP4Info, /*egress_rif_vlan=*/std::nullopt, kEgressPortProto,
      /*disable_vlan_checks=*/false, /*disable_vlan_rewrite=*/true));

  // Make ingress and egress ports tagged member of vlan.
  ASSERT_OK_AND_ASSIGN(
      std::vector<p4::v1::Entity> pi_entities,
      pdpi::IrEntitiesToPi(
          kIrP4Info,
          GetVlanMembershipEntities(
              {{
                   .vlan_id = kIngressVlanId,
                   .port = kIngressPortProto,
                   .tagging_mode = VlanTaggingMode::kTagged,
               },
               {
                   .vlan_id = kIngressVlanId,
                   .port = kEgressPortProto,
                   .tagging_mode = VlanTaggingMode::kTagged,
               }}),
          pdpi::TranslationOptions{.allow_unsupported = true}));
  ASSERT_OK(pdpi::InstallPiEntities(bmv2.P4RuntimeSession(), pi_entities));

  {
    ASSERT_OK_AND_ASSIGN(
        auto outputs,
        bmv2.SendPacket(kIngressPort, GetVlanIpv4PacketOrDie(kIngressVlanId)));
    ASSERT_THAT(outputs, ElementsAre(Key(kEgressPort)));
    ASSERT_THAT(outputs[kEgressPort].packets(), SizeIs(1));
    ASSERT_THAT(outputs[kEgressPort].packets().at(0).headers(),
                ElementsAre(HasHeaderCase(packetlib::Header::kEthernetHeader),
...
TEST(VlanMembershipTest,
     EgressPacketIsUntaggedIfEgressPortIsUntaggedMemberOfVlanInEgress) {
  const sai::Instantiation kInstantiation = sai::Instantiation::kExperimentalTor;
  const pdpi::IrP4Info kIrP4Info = sai::GetIrP4Info(kInstantiation);
  ASSERT_OK_AND_ASSIGN(Bmv2 bmv2, sai::SetUpBmv2ForSaiP4(kInstantiation));

  constexpr absl::string_view kIngressVlanId = "0xfff";  // VID 4095.
  ...
  // Make ingress and egress ports untagged member of vlan.
  ASSERT_OK_AND_ASSIGN(
      std::vector<p4::v1::Entity> pi_entities,
      pdpi::IrEntitiesToPi(
          kIrP4Info,
          GetVlanMembershipEntities(
              {{
                   .vlan_id = kIngressVlanId,
                   .port = kIngressPortProto,
                   .tagging_mode = VlanTaggingMode::kUntagged,
               },
               {
                   .vlan_id = kIngressVlanId,
                   .port = kEgressPortProto,
  ...
  ASSERT_THAT(outputs, ElementsAre(Key(kEgressPort)));
  ASSERT_THAT(outputs[kEgressPort].packets(), SizeIs(1));
  ASSERT_THAT(outputs.at(kEgressPort).packets().at(0).headers(),
              ElementsAre(HasHeaderCase(packetlib::Header::kEthernetHeader),
                          HasHeaderCase(packetlib::Header::kIpv4Header)));
}

TEST(VlanMembershipTest,
     EgressPacketGetsDropsIfEgressPortIsNotAMemberOfVlanInEgress) {
  const sai::Instantiation kInstantiation = sai::Instantiation::kExperimentalTor;
  const pdpi::IrP4Info kIrP4Info = sai::GetIrP4Info(kInstantiation);
  ASSERT_OK_AND_ASSIGN(Bmv2 bmv2, sai::SetUpBmv2ForSaiP4(kInstantiation));

  constexpr absl::string_view kIngressVlanId = "0x00a";  // VID 10.
  // Carry over VLAN in ingress to egress.
  ASSERT_OK(InstallEntriesForwardingAndRewritingVlanInRifTable(
      bmv2, kIrP4Info, /*egress_rif_vlan=*/std::nullopt, kEgressPortProto,
      /*disable_vlan_checks=*/false, /*disable_vlan_rewrite=*/true));

  // Make ingress port a member of vlan.
  ASSERT_OK_AND_ASSIGN(
      std::vector<p4::v1::Entity> pi_entities,
      pdpi::IrEntitiesToPi(
          kIrP4Info,

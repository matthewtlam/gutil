// Package gpinsbind contains all the code related to the GPINS project's binding to Ondatra.
package gpinsbind
...
import (
	...
	gpinsbackend "github.com/pins/pins-infra/ondatra/binding/gpinsbackend"
	...
	"github.com/pins/pins-infra/ondatra/binding/bindingbackend"
...
var (
	supportedBackends = []string{"gpins"}
	backendType       = flag.String("gpins_backend_type", "gpins", fmt.Sprintf("define the gpins-ondatra backend type, choose from : %v. Uses gpins as default.", supportedBackends))
)
...
// Binding is a binding for GPINS switches.
type Binding struct {
...
// New returns a new instance of a GPINS Binding.
...
func createBackendFromFlag() (bindingbackend.Backend, error) {
	switch *backendType {
	case "gpins":
		log.Infof("Creating GPINS backend")
		return gpinsbackend.New(), nil
	default:
		return nil, fmt.Errorf("unknown backend type: %q", *backendType)
	}
}

// NewWithOpts returns a new instance of a GPINS Binding.
...
func (b *Binding) Reserve(ctx context.Context, tb *opb.Testbed, runtime, waitTime time.Duration, partial map[string]string) (*binding.Reservation, error) {
	if len(partial) > 0 {
		return nil, fmt.Errorf("GPINSBind Reserve does not yet support partial mappings")
	}
...
type gpinsDUT struct {
	*binding.AbstractDUT
	bind *Binding
	grpc bindingbackend.GRPCServices
}

type gpinsATE struct {
	*binding.AbstractATE
	bind *Binding
...
func (d *gpinsDUT) dialGRPC(ctx context.Context, grpcInfo bindingbackend.ServiceInfo, opts ...grpc.DialOption) (*grpc.ClientConn, error) {
	addr := grpcInfo.Addr
	if addr == "" {
...
func (d *gpinsDUT) DialGNMI(ctx context.Context, opts ...grpc.DialOption) (gpb.GNMIClient, error) {
...
// Calling the CloseSend will cause GPINs to close the Subscribe stream and return an
// error (b/219112957). Hence we overwrite this method to be no-op here.
...
func (d *gpinsDUT) DialGNOI(ctx context.Context, opts ...grpc.DialOption) (gnoigo.Clients, error) {
...
func (d *gpinsDUT) DialP4RT(ctx context.Context, opts ...grpc.DialOption) (p4pb.P4RuntimeClient, error) {
...
func (d *gpinsDUT) DialConsole(ctx context.Context) (binding.ConsoleClient, error) {
...
func (b *Binding) Resolve() (*rpb.Reservation, error) {
	...
	for k, d := range b.resv.DUTs {
		rD, err := b.resolveDUT(k, d.(*gpinsDUT))
		if err != nil {
			return nil, err
		}
		devices[k] = rD
	...
	for k, a := range b.resv.ATEs {
		rD, err := b.resolveATE(k, a.(*gpinsATE))
		if err != nil {
			return nil, err
		}
		ates[k] = rD
...
func (b *Binding) resolveDUT(key string, d *gpinsDUT) (*rpb.ResolvedDevice, error) {
...
func (b *Binding) resolveATE(key string, d *gpinsATE) (*rpb.ResolvedDevice, error) {
	ports := map[string]*rpb.ResolvedPort{}
	for k, p := range d.Ports() {
		ports[k] = resolvePort(k, p)
	}

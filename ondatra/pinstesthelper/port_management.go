...
type PortProperties struct {
	index                  int
	supportedSpeeds        map[oc.E_TransportTypes_ETHERNET_PMD_TYPE]map[int][]oc.E_IfEthernet_ETHERNET_SPEED
	defaultBreakoutMode    string
	supportedBreakoutModes map[oc.E_TransportTypes_ETHERNET_PMD_TYPE][]string
	mediaType              string
}
...
func CollateralFlapAllowed(t *testing.T, dut *ondatra.DUTDevice, pmdType oc.E_TransportTypes_ETHERNET_PMD_TYPE) (bool, error) {
	info, err := portInfoForDevice(t, dut)
	if err != nil {
		return false, errors.Wrap(err, "failed to fetch platform specific information")
	}
	if allowed, ok := info.collateralFlap[pmdType]; ok {
		return allowed, nil
	}
	// Assume collateral flap is not allowed if entry doesn't exist!
	log.Infof("Update collateralFlap map to include PMD type: %v!", pmdType.String())
	return false, nil
}
...
func FrontPanelPortToIndexMappingForDevice(t *testing.T, dut *ondatra.DUTDevice) (map[string]int, error) {
	...
	for port, value := range info.portInfo {
...
func SupportedSpeedsForPort(t *testing.T, dut *ondatra.DUTDevice, interfaceName string) ([]oc.E_IfEthernet_ETHERNET_SPEED, error) {
	...
	port, ok := info.portInfo[interfaceName]
	if !ok {
		return nil, errors.Errorf("no entry found for interface %v in front panel port list", interfaceName)
	}
	lanes := len(pinstesthelperIntfPhysicalChannelsGet(t, dut, interfaceName))
	pmdType, err := pinstesthelperPortPmdTypeGet(t, dut, interfaceName)
	if err != nil {
		return nil, err
	}
	if _, ok := port.supportedSpeeds[pmdType][lanes]; !ok {
		return nil, errors.Errorf("no supported speeds found for interface %v pmd %v with %v lanes", interfaceName, pmdType.String(), lanes)
	}
	return port.supportedSpeeds[pmdType][lanes], nil
}
...
func TransceiverEmpty(t *testing.T, d *ondatra.DUTDevice, port string) (bool, error) {
	...
	return pinstesthelperTransceiverEmpty(t, d, FrontPanelPortPrefix+strconv.Itoa(transceiverNumber)), nil
}
...
func CurrentBreakoutModeForPort(t *testing.T, dut *ondatra.DUTDevice, port string) (string, error) {
	...
	groups := pinstesthelperBreakoutModeGet(t, dut, physicalPort)
	if groups == nil {
...
func SupportedBreakoutModesForPort(t *testing.T, dut *ondatra.DUTDevice, interfaceName string, breakoutType BreakoutType) ([]string, error) {
	...
	if port.supportedBreakoutModes == nil {
		return nil, errors.Errorf("no supported breakout modes found for port %v", interfaceName)
	}
	pmdType, err := pinstesthelperPortPmdTypeGet(t, dut, interfaceName)
	if err != nil {
		return nil, err
	}
	if _, ok := port.supportedBreakoutModes[pmdType]; !ok {
		return nil, errors.Errorf("unsupported port PMD type (%v) found", pmdType)
	}
	// Return requested type of breakout modes only.
	var supportedBreakoutModesOfBreakoutType []string
	if breakoutType == Mixed {
		for _, mode := range port.supportedBreakoutModes[pmdType] {
			if strings.Contains(mode, "+") {
				supportedBreakoutModesOfBreakoutType = append(supportedBreakoutModesOfBreakoutType, mode)
			}
		}
		port.supportedBreakoutModes[pmdType] = supportedBreakoutModesOfBreakoutType
	}
	...
	return port.supportedBreakoutModes[pmdType], nil
}
...
func computePortIDForPort(t *testing.T, d *ondatra.DUTDevice, intfName string) (uint32, error) {
	...
	var id int
	id, err := pinstesthelperPortIDGet(t, d, intfName)
...
func ConfigFromBreakoutMode(t *testing.T, dut *ondatra.DUTDevice, breakoutMode, port string) (*oc.Root, error) {
	...
	if err != nil {
		return nil, errors.Errorf("failed to fetch front panel port to index mapping from device %v", pinstesthelperDUTNameGet(dut))
	}
...
func BreakoutStateInfoForPort(t *testing.T, dut *ondatra.DUTDevice, port string, currBreakoutMode string) (map[string]*PortBreakoutInfo, error) {
	...
	for p := range portInfo {
		physicalChannels := pinstesthelperIntfPhysicalChannelsGet(t, dut, p)
		operStatus := pinstesthelperIntfOperStatusGet(t, dut, p)
		portSpeed := pinstesthelperStatePortSpeedGet(t, dut, p)
...
func WaitForInterfaceState(t *testing.T, dut *ondatra.DUTDevice, intfName string, expectedOperSatus oc.E_Interface_OperStatus, timeout time.Duration) error {
	...
	for start := time.Now(); time.Since(start) < timeout; {
		if got = pinstesthelperIntfOperStatusGet(t, dut, intfName); got == expectedOperSatus {
			return nil
		}
...
func TransceiverNumberForPort(t *testing.T, dut *ondatra.DUTDevice, port string) (int, error) {
	...
	// transceiver number.
	prefix := "1/"
	hardwarePort := pinstesthelperIntfHardwarePortGet(t, dut, port)
...
func PortPMDFromModel(t *testing.T, dut *ondatra.DUTDevice, port string) (oc.E_TransportTypes_ETHERNET_PMD_TYPE, error) {
	return pinstesthelperPortPmdTypeGet(t, dut, port)
}

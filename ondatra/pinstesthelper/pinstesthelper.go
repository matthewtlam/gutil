...
var (
	...
	pinstesthelperPortPmdTypeGet = func(t *testing.T, d *ondatra.DUTDevice, port string) (oc.E_TransportTypes_ETHERNET_PMD_TYPE, error) {
		...
		return pph.portPmdType(d.Name(), port)
	}
...
type PortInfo struct {
	maxLanes       int
	portInfo       map[string]PortProperties
	collateralFlap map[oc.E_TransportTypes_ETHERNET_PMD_TYPE]bool
}
...
func platformInfoForDevice(t *testing.T, dut *ondatra.DUTDevice) (*PlatformInfo, error) {
	...
	sysInfo := SystemInfo{
		rebootTime: 15 * time.Minute,
	}

	return &PlatformInfo{
		systemInfo: sysInfo,
	}, nil
	return nil, errors.Errorf("unimplemented function")
}
